 ...
 * Updated to return error codes for all I/O and allocation operations.
 * Refactored macros to safely handle control flow (do-while(0)).
 * @author Samuel Marks
 */
...
/* Helper macro for fprintf error checking wrapped in do-while to swallow
 * semicolon */
...
int write_enum_to_str_func(FILE *cfile, const char *const enum_name,
  ...
  CHECK_FPRINTF(fprintf(cfile, "    case %s_UNKNOWN:\n", enum_name));
  CHECK_FPRINTF(fputs("    default:\n"
...
int write_enum_from_str_func(FILE *cfile, const char *const enum_name,
                             const struct EnumMembers *em) {
  size_t i;
  const char *member;
  if (!cfile || !enum_name || !em || !em->members)
  ...
  CHECK_FPRINTF(fprintf(cfile, "  else *val = %s_UNKNOWN;\n", enum_name));
  CHECK_FPRINTF(fputs("  return 0;\n"
                      "}\n",
                      cfile));
  return 0;
}

int write_struct_from_jsonObject_func(FILE *cfile,
  ...
  CHECK_FPRINTF(fprintf(cfile, "  struct %s *ret;\n", struct_name));

  for (i = 0; i < sf->size; i++) {
    f = &sf->fields[i];
    if (strcmp(f->type, "enum") == 0) {
      CHECK_FPRINTF(fprintf(cfile, "  const char *_%s_str = NULL;\n", f->name));
  ...
  for (i = 0; i < sf->size; i++) {
    ...
    if (strcmp(f->type, "string") == 0) {
      ...
      CHECK_FPRINTF(fprintf(
          cfile, "  ret->%s = json_object_get_number(jsonObject, \"%s\");\n",
          f->name, f->name));
      ...
      CHECK_FPRINTF(
          fprintf(cfile,
                  ...
                  f->name, f->name, f->name, struct_name,
                  get_type_from_ref(f->ref), f->name, f->name, struct_name));
...
int write_struct_from_json_func(FILE *cfile, const char *const struct_name) {
...
int write_struct_to_json_func(FILE *cfile, const char *const struct_name,
  ...
  CHECK_FPRINTF(fputs("  int need_comma = 0;\n"
  ...
  CHECK_FPRINTF(fputs("  jasprintf(json, \"}\");\n"
                      "  if (*json == NULL) return ENOMEM;\n"
                      "  return 0;\n"
                      "}\n",
                      cfile));
...
int write_struct_cleanup_func(FILE *cfile, const char *struct_name,
                              const struct StructFields *sf) {
  size_t i;
  const struct StructField *f;
  if (!cfile || !struct_name || !sf)
  ...
  CHECK_FPRINTF(fprintf(cfile, "void %s_cleanup(struct %s *const obj) {\n",
                        struct_name, struct_name));
  CHECK_FPRINTF(fprintf(cfile, "  if (obj == NULL) return;\n"));
  ...
  for (i = 0; i < sf->size; i++) {
    ...
    if (strcmp(f->type, "string") == 0) {
      ...
      CHECK_FPRINTF(fprintf(cfile, "  %s_cleanup(obj->%s);\n",
...
int write_struct_default_func(FILE *cfile, const char *struct_name,
                              const struct StructFields *sf) {
  size_t i;
  const struct StructField *f;
  if (!cfile || !struct_name || !sf)
    return EINVAL;
  CHECK_FPRINTF(fprintf(cfile, "int %s_default(struct %s **out) {\n",
                        struct_name, struct_name));
  CHECK_FPRINTF(fputs("  if (!out) return EINVAL;\n", cfile));
  CHECK_FPRINTF(fprintf(cfile, "  *out = malloc(sizeof(**out));\n"
  ...
  for (i = 0; i < sf->size; i++) {
    f = &sf->fields[i];
    if (strcmp(f->type, "string") == 0) {
      CHECK_FPRINTF(fprintf(cfile, "  (*out)->%s = NULL;\n", f->name));
    } else if (strcmp(f->type, "integer") == 0 ||
               strcmp(f->type, "boolean") == 0 ||
               strcmp(f->type, "number") == 0) {
      CHECK_FPRINTF(fprintf(cfile, "  (*out)->%s = 0;\n", f->name));
    } else if (strcmp(f->type, "enum") == 0) {
      CHECK_FPRINTF(fprintf(cfile, "  (*out)->%s = %s_UNKNOWN;\n", f->name,
  ...
  CHECK_FPRINTF(fprintf(cfile, "  return 0;\n}\n\n"));
  return 0;
}
...
int write_struct_deepcopy_func(FILE *cfile, const char *struct_name,
  ...
  if (!cfile || !struct_name || !sf)
    return EINVAL;
  CHECK_FPRINTF(fprintf(
      cfile, "int %s_deepcopy(const struct %s *src, struct %s **dest) {\n",
      struct_name, struct_name, struct_name));
  CHECK_FPRINTF(fputs("  if (!dest) return EINVAL;\n", cfile));
  CHECK_FPRINTF(
