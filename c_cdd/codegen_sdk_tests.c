...
static int write_test_operation(FILE *fp, const struct OpenAPI_Operation *op,
  ...
  /* Error out assumed not used in simple signature unless configured, currently
   * codegen_client_sig handles it if present */
  /* Checking if error responses exist */
  int has_err = 0;
  for (j = 0; j < op->n_responses; ++j) {
    if (op->responses[j].code[0] == '4' && op->responses[j].schema.ref_name) {
      has_err = 1;
    }
  }
  if (has_err) {
  ...
  CHECK_IO(fprintf(fp, "  /* ASSERT_EQ(0, rc); Intentionally commented out as "
