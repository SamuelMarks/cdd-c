...
This directory previously contained a standalone Python `tree-sitter` powered script for refactoring C `void` or `bool` return types into `cdd_c_error_t` based error percolation chains (`cdd_c_error_t rc = ...; if (rc != CDD_C_SUCCESS) return rc;`).
