...
def main():
    ...
    if job == "build":
        ...
        run_cmd(
            ["cmake", "..", "-DCMAKE_EXPORT_COMPILE_COMMANDS=ON"], cwd="build_cmake"
        )
        ...
        if os.path.exists("build_cmake/compile_commands.json"):
            shutil.copy("build_cmake/compile_commands.json", "compile_commands.json")

    elif job == "audit":
        print("=== Auditing with Transformer ===")
        cdd_c_bin = (
            os.path.join("bin", "cdd-c.exe")
            if os.name == "nt"
            else os.path.join("bin", "cdd-c")
        )
        if not os.path.exists(cdd_c_bin):
            print(f"Warning: {cdd_c_bin} not found. Run build first. Skipping audit.")
            sys.exit(0)
        c_files = []
                ...
                for dp, _, fns in os.walk(d):
                    for fn in fns:
                        if fn.endswith(".c") or fn.endswith(".h"):
                            c_files.append(os.path.join(dp, fn))
        if not c_files:
        ...
        if sys.platform.startswith("linux"):
            ...
            if not os.path.exists(cdd_c_bin):
                ...
                try:
                    ...
                    print(
                        "Warning: valgrind is not installed or failed. Skipping valgrind."
                    )
        else:
        ...
        print(
            "WASM build skipped in native Windows pre-commit to avoid wasi-sdk download complexities. Use GitHub Actions for WASM validation."
        )
        ...
        if has_java:
            ...
            oas3_ok = download_if_missing(
                "https://jitpack.io/com/github/OpenAPITools/openapi-petstore/master-SNAPSHOT/openapi-petstore-master-SNAPSHOT.jar",
                oas3_jar,
            )
            sw2_ok = download_if_missing(
                "https://jitpack.io/com/github/swagger-api/swagger-petstore/master-SNAPSHOT/swagger-petstore-master-SNAPSHOT.war",
                sw2_war,
            )
            jetty_ok = download_if_missing(
            ...
            if oas3_ok and sw2_ok and jetty_ok:
                ...
                sw2_proc = subprocess.Popen(
                    ["java", "-jar", jetty_jar, "--port", "8092", sw2_war],
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.DEVNULL,
                )
                oas3_proc = subprocess.Popen(
                    [
                        "java",
                        "--add-opens",
                ...
                for _ in range(30):
                    try:
                        ...
                        urllib.request.urlopen(
                            "http://localhost:8093/api/v3/openapi.json"
                        )
                        jvm_started = True
                        break
                    except Exception:
                ...
                if not jvm_started:
                    print(
                        "Warning: JVM Petstore servers failed to start or become ready in time. Falling back..."
                    )
                    sw2_proc.kill()
                    oas3_proc.kill()
            else:
        ...
        if not jvm_started and has_docker:
            ...
            run_cmd(
                ["docker", "rm", "-f", "petstore_sw2_test", "petstore_oas3_test"],
                check=False,
            )
            run_cmd(
                [
                    ...
                    "petstore_sw2_test",
                    "-e",
                    "SWAGGER_HOST=http://localhost:8092",
                    "-e",
            ...
            run_cmd(
                [
                    ...
                    "petstore_oas3_test",
                    "-e",
                    "SWAGGER_HOST=http://localhost:8093",
                    "-e",
        ...
        if jvm_started:
            ...
            try:
                ...
                urllib.request.urlretrieve(
                    "http://localhost:8092/api/swagger.json", spec_sw2
                )
            except Exception as e:
            ...
            try:
                ...
                urllib.request.urlretrieve(
                    "http://localhost:8093/api/v3/openapi.json", spec_oas3
                )
            except Exception as e:
            ...
            try:
                ...
                urllib.request.urlretrieve(
                    "http://localhost:8092/v2/swagger.json", spec_sw2
                )
            except Exception as e:
                print(f"Warning: Failed to fetch Swagger 2.0 spec: {e}")
            ...
            try:
                ...
                try:
                    ...
                    urllib.request.urlretrieve(
                        "http://localhost:8093/api/v3/swagger.json", spec_oas3
                    )
            except Exception as e:
                print(f"Warning: Failed to fetch OAS3 spec: {e}")
        ...
        if os.path.exists(spec_oas3) and os.path.exists(cdd_c_bin):
            ...
            run_cmd(
                [
                    ...
                    "-DFETCHCONTENT_SOURCE_DIR_CDD_C=..",
                ],
                cwd="test_out_oas3",
            )
            ...
            print(
                f"Warning: Spec {spec_oas3} or binary {cdd_c_bin} not found. Skipping OAS3 test."
            )
        ...
        if os.path.exists(spec_sw2) and os.path.exists(cdd_c_bin):
            ...
            print(
                f"Warning: Spec {spec_sw2} or binary {cdd_c_bin} not found. Skipping Swagger 2 test."
            )
