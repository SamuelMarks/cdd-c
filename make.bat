...
mkdir build_wasm
cd build_wasm
cmake .. -G "MinGW Makefiles" -DCMAKE_TOOLCHAIN_FILE=../wasi-sdk/share/cmake/wasi-sdk.cmake -DCMAKE_C_FLAGS="-DCFS_OS_DOS=1 -include sys/types.h -include unistd.h -D_WASI_EMULATED_GETPID -D_WASI_EMULATED_SIGNAL -D_WASI_EMULATED_PROCESS_CLOCKS" -DCMAKE_EXE_LINKER_FLAGS="-lwasi-emulated-getpid -lwasi-emulated-signal -lwasi-emulated-process-clocks" -DC_CDD_USE_LIBCURL=OFF -DHTTP_ONLY=ON -DC_CDD_THREADING="Single-threaded" -DENABLE_THREADED_RESOLVER=OFF -DCURL_DISABLE_LDAP=ON -DCURL_DISABLE_LDAPS=ON -DC_ABSTRACT_HTTP_USE_RAW_SOCKETS=ON -DBUILD_TESTING=OFF -DFETCHCONTENT_UPDATES_DISCONNECTED=ON -DC_CDD_BUILD_TESTS=OFF -DC_ABSTRACT_HTTP_BUILD_TESTING=OFF -DC_ORM_BUILD_TESTING=OFF -DC_FS_BUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release
powershell -Command "(Get-Content _deps\c_rest_framework-src\CMakeLists.txt -ErrorAction Ignore) -replace 'STREQUAL \"DOS\"', 'STREQUAL \"WASI\" OR CMAKE_SYSTEM_NAME STREQUAL \"DOS\"' | Set-Content _deps\c_rest_framework-src\CMakeLists.txt -ErrorAction Ignore"
cmake .. -G "MinGW Makefiles" -DCMAKE_TOOLCHAIN_FILE=../wasi-sdk/share/cmake/wasi-sdk.cmake -DCMAKE_C_FLAGS="-DCFS_OS_DOS=1 -include sys/types.h -include unistd.h -D_WASI_EMULATED_GETPID -D_WASI_EMULATED_SIGNAL -D_WASI_EMULATED_PROCESS_CLOCKS" -DCMAKE_EXE_LINKER_FLAGS="-lwasi-emulated-getpid -lwasi-emulated-signal -lwasi-emulated-process-clocks" -DC_CDD_USE_LIBCURL=OFF -DHTTP_ONLY=ON -DC_CDD_THREADING="Single-threaded" -DENABLE_THREADED_RESOLVER=OFF -DCURL_DISABLE_LDAP=ON -DCURL_DISABLE_LDAPS=ON -DC_ABSTRACT_HTTP_USE_RAW_SOCKETS=ON -DBUILD_TESTING=OFF -DFETCHCONTENT_UPDATES_DISCONNECTED=ON -DC_CDD_BUILD_TESTS=OFF -DC_ABSTRACT_HTTP_BUILD_TESTING=OFF -DC_ORM_BUILD_TESTING=OFF -DC_FS_BUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=Release
cmake --build . --target cdd-c
cd ..
if not exist "bin" mkdir bin
copy build_wasm\bin\cdd-c bin\cdd-c.wasm >nul 2>&1
if errorlevel 1 copy build_wasm\cdd-c bin\cdd-c.wasm >nul 2>&1
goto end
