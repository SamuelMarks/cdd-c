 ...
 *
 * Injects `\#ifdef <condition>`, an optional `\#else` block with
 * `false_tokens`, and `\#endif` around the tokens in the range `[start_idx,
 * end_idx)`. Applies correct local indentation to the injected macros.
 *
 ...
 * the requested MSVC headers (e.g., `<windows.h>`) after it within an `\#ifdef
 * _MSC_VER` block. Automatically injects `#define WIN32_LEAN_AND_MEAN` and
 * `#define NOMINMAX` before `<windows.h>`, and ensures `<winsock2.h>` precedes
 * `<windows.h>`.
 *
 * @param[in,out] patches The patch list to populate.
 * @param[in] tokens The original token stream.
 ...
 * @param[in] size_expr The string representation of the size expression (e.g.
 * "n").
