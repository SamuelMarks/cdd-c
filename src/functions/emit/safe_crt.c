 ...
 */
static cdd_c_error_t add_patch(struct SafeCrtPatchList *list, size_t start,
 ...
 */
static cdd_c_error_t extract_token_text(const struct TokenList *tokens,
 ...
 */
static cdd_c_error_t generate_strcpy_patch(const struct TokenList *tokens,
  ...
  for (i = call_start; i < call_end; ++i) {
    if (tokens->tokens[i].kind == TOKEN_LPAREN && lparen == 0)
      lparen = i;
    else if (tokens->tokens[i].kind == TOKEN_COMMA && lparen != 0 && comma == 0)
      comma = i;
    else if (tokens->tokens[i].kind == TOKEN_RPAREN && comma != 0 &&
             rparen == 0)
      rparen = i;
  }
 ...
 */
static cdd_c_error_t generate_fopen_patch(const struct TokenList *tokens,
                                          size_t call_start, size_t call_end,
                                          struct SafeCrtPatchList *out) {
...
Find the assignment target to rewrite it as fopen_s(&f, path, mode);
  */
  ...
  for (i = call_start; i < call_end; ++i) {
    if (tokens->tokens[i].kind == TOKEN_LPAREN && lparen == 0)
      lparen = i;
    else if (tokens->tokens[i].kind == TOKEN_COMMA && comma1 == 0)
      comma1 = i;
 ...
 */
static cdd_c_error_t generate_strncpy_patch(const struct TokenList *tokens,
                                            size_t call_start, size_t call_end,
                                            struct SafeCrtPatchList *out) {
  ...
  for (i = call_start; i < call_end; ++i) {
    if (tokens->tokens[i].kind == TOKEN_LPAREN && lparen == 0)
      ...
      if (comma1 == 0)
        ...
        comma2 = i;
    } else if (tokens->tokens[i].kind == TOKEN_RPAREN && rparen == 0)
      rparen = i;
  }
 ...
 */
static cdd_c_error_t generate_sprintf_patch(const struct TokenList *tokens,
                                            size_t call_start, size_t call_end,
                                            struct SafeCrtPatchList *out) {
  ...
  for (i = 0; i < cst->size; ++i) {
    ...
    for (j = n->start_token; j < n->end_token; ++j) {
      if ((tokens->tokens[j].kind == TOKEN_KEYWORD_CHAR ||
        ...
        if (tokens->tokens[j + 1].kind == TOKEN_IDENTIFIER &&
            tokens->tokens[j + 2].kind == TOKEN_LBRACKET) {
