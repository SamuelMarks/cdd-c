 ...
 * targets and safely append MSVC-specific target_compile_options and link libraries.
 *
 * @author Samuel Marks
 */
 ...
 * @param[in] target_name Optional target name. Pass NULL to apply globally via add_compile_options/link_libraries.
 ...
 */
extern C_CDD_EXPORT int cmake_modifier_add_compile_opt(struct CMakeModifier *mod,
                                                       const char *opt);

/**
 * @brief Add a link library to be injected.
 */
extern C_CDD_EXPORT int cmake_modifier_add_link_lib(struct CMakeModifier *mod,
                                                    const char *lib);
 ...
 *
 * Scans the file. If `target_name` is set, looks for `target_compile_options(target_name ...)`
 * and `target_link_libraries(target_name ...)`. If found, appends the new values within an
 * `if(MSVC)` block. If not found, appends new commands to the end of the file.
 * Creates a `.patch` file instead of direct modification.
 *
 * @param[in] mod The configured modifier.
 * @param[out] out_diff A newly allocated string containing the unified diff.
 * @return 0 on success.
 */
extern C_CDD_EXPORT int cmake_modifier_apply_diff(const struct CMakeModifier *mod,
                                                  char **out_diff);
