 ...
 *
 * Scans legacy build scripts to extract source files, include directories
 * (`-I`), and compile definitions (`-D`) to bootstrap modern CMakeLists.txt.
 *
 * @author Samuel Marks
 */
 ...
 *
 * VERY basic heuristic scanner. Looks for lines containing `.c`, `-I`, and
 * `-D`. It is not a full Make parser.
 *
 ...
 * @return 0 on success.
 */
extern C_CDD_EXPORT cdd_c_error_t scrape_configure_ac(
    struct ExtractedBuildInfo *info, const char *configure_ac_content);
 ...
 * @param[out] out_cmake Newly allocated string containing the CMake file
 * contents.
 * @return 0 on success.
 */
extern C_CDD_EXPORT cdd_c_error_t
