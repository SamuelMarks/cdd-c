 ...
 * The `expanded_ast` contains the parsed C tokens representing the semantic
 * evaluation.
 */
struct MacroOverlayNode {
  const struct CstNode *invocation_node; /**< The raw textual CST node
 ...
 */
C_CDD_EXPORT /**
              * @brief Executes the macro overlay list free operation.
              */
    void
    macro_overlay_list_free(struct MacroOverlayList *list);
