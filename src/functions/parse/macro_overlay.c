...
static int list_add(struct MacroOverlayList *list, const struct CstNode *node, struct CstNodeList *expanded) {
  if (!list || !node) return EINVAL;
  
  if (list->size >= list->capacity) {
    size_t new_cap = list->capacity == 0 ? 8 : list->capacity * 2;
    struct MacroOverlayNode *new_arr = realloc(list->nodes, new_cap * sizeof(struct MacroOverlayNode));
...
int cst_build_macro_overlay(const struct CstNodeList *cst,
  ...
  size_t i;
  if (!cst || !tokens || !overlays) return EINVAL;

  /* Traverse CST to find CST_NODE_MACRO */
  for (i = 0; i < cst->size; ++i) {
    const struct CstNode *n = &cst->nodes[i];
    if (n->kind == CST_NODE_MACRO) {
      /* Create a dummy expanded list for now, since actual preprocessor evaluation 
         is a complex step that requires building a full env. */
      struct CstNodeList *dummy_expanded = calloc(1, sizeof(struct CstNodeList));
      if (!dummy_expanded) return ENOMEM;
