...
EM_JS(int, js_crypto_hmac_sha256,
        ...
        if (typeof process != = 'undefined' && process.versions &&
          ...
          try {
            ...
            var hmac = crypto.createHmac(
                'sha256', new Uint8Array(HEAPU8.buffer, key, key_len));
...
static cdd_c_error_t cdd_sha256_init(struct cdd_sha256_ctx *ctx) {
  ctx->datalen = 0;
  ctx->bitlen = 0;
...
static cdd_c_error_t cdd_sha256_update(struct cdd_sha256_ctx *ctx,
  ...
  for (i = 0; i < len; ++i) {
    ...
    if (ctx->datalen == 64) {
      {
        cdd_c_error_t rc_cr = cdd_sha256_transform(ctx, ctx->data);
...
static cdd_c_error_t cdd_sha256_final(struct cdd_sha256_ctx *ctx,
                                      uint8_t hash[]) {
  uint32_t i;
  i = ctx->datalen;
  if (ctx->datalen < 56) {
