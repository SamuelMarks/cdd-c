...
static cdd_c_error_t cdd_sha256_transform(struct cdd_sha256_ctx *ctx, const uint8_t data[]) {
...
static cdd_c_error_t cdd_sha256_update(struct cdd_sha256_ctx *ctx, const uint8_t data[], size_t len) {
  size_t i;
  for (i = 0; i < len; ++i) {
...
static cdd_c_error_t cdd_sha256_final(struct cdd_sha256_ctx *ctx, uint8_t hash[]) {
  uint32_t i;
  i = ctx->datalen;
  if (ctx->datalen < 56) {
    ctx->data[i++] = 0x80;
