...
void desig_init_list_free(struct DesigInitList *list) {
  ...
  if (list->sites) {
    for (i = 0; i < list->count; i++) {
      if (list->sites[i].field_name) free(list->sites[i].field_name);
      if (list->sites[i].value_expr) free(list->sites[i].value_expr);
    }
    free(list->sites);
  }
...
int scan_for_designated_initializers(const struct TokenList *tokens, struct DesigInitList *list) { char * _ast_strndup_0; char * _ast_strndup_1;  char * ; char * ;  size_t i = 0;
  ...
  while (i < tokens->size) {
    if (tokens->tokens[i].kind == TOKEN_LBRACE) {
      if (brace_depth >= brace_cap) {
        ...
        brace_stack = (size_t *)realloc(brace_stack, brace_cap * sizeof(size_t));
        if (!brace_stack) {
          res = ENOMEM;
          goto cleanup;
        }
      ...
      if (look < tokens->size && tokens->tokens[look].kind == TOKEN_IDENTIFIER) {
        ...
        if (look < tokens->size && tokens->tokens[look].kind == TOKEN_ASSIGN) {
          ...
          while (look < tokens->size) {
            if (tokens->tokens[look].kind == TOKEN_LBRACE) {
              ...
              if (nested_braces == 0) break; /* End of struct initialization */
              nested_braces--;
            } else if (tokens->tokens[look].kind == TOKEN_COMMA && nested_braces == 0) {
          ...
          while (expr_end > expr_start && tokens->tokens[expr_end - 1].kind == TOKEN_WHITESPACE) {
            expr_end--;
          }

          if (list->count >= list->capacity) {
            list->capacity = list->capacity == 0 ? 4 : list->capacity * 2;
            list->sites = (struct DesigInitSite *)realloc(list->sites, list->capacity * sizeof(struct DesigInitSite));
            if (!list->sites) {
              res = ENOMEM;
              goto cleanup;
            }
...

          {
            ...
            s->brace_end_idx = 0; /* Will be resolved later if needed, hard to know exact end in forward pass without full parse */
            
            s->field_name = ((c_cdd_strndup((const char *)tokens->tokens[ident_idx].start, tokens->tokens[ident_idx].length, &, &_ast_strndup_0), _ast_strndup_0), );
            
            {
              const char *e_start = (const char *)tokens->tokens[expr_start].start;
              const char *e_end = (const char *)tokens->tokens[expr_end - 1].start + tokens->tokens[expr_end - 1].length;
              s->value_expr = ((c_cdd_strndup(e_start, (size_t)(e_end - e_start), &, &_ast_strndup_1), _ast_strndup_1), );
            }
...
cleanup:
  if (brace_stack) free(brace_stack);
  return res;
}