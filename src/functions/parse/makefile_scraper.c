...
static void add_string_to_array(char ***arr, size_t *n, const char *str) { size_t i;
  /* check dupes */
  for (i = 0; i < *n; i++) {
...
int scrape_makefile(struct ExtractedBuildInfo *info, const char *makefile_content) { char *copy;
  char *saveptr;
  char *tok;

  if (!info || !makefile_content) return EINVAL;
...
int scrape_configure_ac(struct ExtractedBuildInfo *info, const char *configure_ac_content) { /* Minimal implementation: scan similarly to Makefile for basic definitions */
  char *copy;
  char *saveptr;
  char *tok;

  if (!info || !configure_ac_content) return EINVAL;
...
int build_info_to_cmake(const struct ExtractedBuildInfo *info,
  ...
  if (!info || !project_name || !out_cmake) return EINVAL;
...
#if defined(_MSC_VER) && !defined(__INTEL_COMPILER)
    len += _snprintf_s(buf + len, cap - len, _TRUNCATE, "add_compile_definitions(\\n");
#else
...
#if defined(_MSC_VER) && !defined(__INTEL_COMPILER)
    len += _snprintf_s(buf + len, cap - len, _TRUNCATE, "target_include_directories(%s PRIVATE\\n", project_name);
#else
    len += snprintf(buf + len, cap - len, "target_include_directories(%s PRIVATE\\n", project_name);
#endif
    for (i = 0; i < info->include_dirs_n; i++) {
#if defined(_MSC_VER) && !defined(__INTEL_COMPILER)
      len += _snprintf_s(buf + len, cap - len, _TRUNCATE, "  %s\\n", info->include_dirs[i]);
