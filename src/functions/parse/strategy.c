 ...
 */
static cdd_c_error_t find_next_token_idx(const struct TokenList *tokens,
 ...
 */
static cdd_c_error_t range_to_string(const struct TokenList *tokens,
                                     size_t start, size_t end, char **out_val) {
  size_t len = 0;
  size_t i;
  char *buf, *p;
  if (!out_val)
