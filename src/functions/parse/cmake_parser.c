 ...
 */
cdd_c_error_t cmake_modifier_init(struct CMakeModifier *mod,
                                  const char *filepath,
 ...
 */
cdd_c_error_t cmake_modifier_add_compile_opt(struct CMakeModifier *mod,
                                             const char *opt) {
  if (!mod || !opt)
    return CDD_C_ERROR_INVALID_ARGUMENT;
 ...
 */
cdd_c_error_t cmake_modifier_add_link_lib(struct CMakeModifier *mod,
                                          const char *lib) {
  if (!mod || !lib)
    return CDD_C_ERROR_INVALID_ARGUMENT;
 ...
 */
static cdd_c_error_t read_file_to_string(const char *filename, size_t *out_len,
                                         char **out_val) {
  FILE *f;
  char *buf = NULL;
  long size;
 ...
 */
cdd_c_error_t cmake_modifier_apply_diff(const struct CMakeModifier *mod,
                                        char **out_diff) {
  size_t len = 0;
  char *src = NULL;
  char *diff;
...
#else
  diff_len +=
      (size_t)CDD_SNPRINTF(diff + diff_len, diff_cap - diff_len,
                           "--- %s\n+++ %s\n", mod->filepath, mod->filepath);
#endif
  ...
  str_buf_len += _snprintf_s(str_buf + str_buf_len, 1024 - str_buf_len,
                             _TRUNCATE, "if(MSVC)\n");
#else
  str_buf_len += (size_t)CDD_SNPRINTF(str_buf + str_buf_len, 1024 - str_buf_len,
                                      "if(MSVC)\n");
#endif
...
#else
      str_buf_len += (size_t)CDD_SNPRINTF(
          str_buf + str_buf_len, 1024 - str_buf_len,
          "    target_compile_options(%s PRIVATE", mod->target_name);
#endif
    } else {
#if defined(_MSC_VER) && !defined(__INTEL_COMPILER)
...
#else
      str_buf_len +=
          (size_t)CDD_SNPRINTF(str_buf + str_buf_len, 1024 - str_buf_len,
                               "    add_compile_options(");
#endif
    }
    for (i = 0; i < mod->compile_opts_n; i++) {
...
#else
      str_buf_len +=
          (size_t)CDD_SNPRINTF(str_buf + str_buf_len, 1024 - str_buf_len, " %s",
                               mod->compile_opts[i]);
#endif
    }
#if defined(_MSC_VER) && !defined(__INTEL_COMPILER)
...
#else
      str_buf_len += (size_t)CDD_SNPRINTF(
          str_buf + str_buf_len, 1024 - str_buf_len, "    link_libraries(");
#endif
    }
    for (i = 0; i < mod->link_libs_n; i++) {
#if defined(_MSC_VER) && !defined(__INTEL_COMPILER)
...
#else
      str_buf_len += (size_t)CDD_SNPRINTF(
          str_buf + str_buf_len, 1024 - str_buf_len, " %s", mod->link_libs[i]);
#endif
    }
#if defined(_MSC_VER) && !defined(__INTEL_COMPILER)
    str_buf_len += _snprintf_s(str_buf + str_buf_len, 1024 - str_buf_len,
