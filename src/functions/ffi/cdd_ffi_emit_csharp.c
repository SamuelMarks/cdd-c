...
static int emit_csharp_bindings(cdd_ffi_ir_t *ir,
...
static int emit_csharp_tests(cdd_ffi_ir_t *ir,
                             const cdd_generate_bindings_config_t *config) {
  FILE *f;
  char filepath[1024];
  size_t i;
...
static int emit_csproj(const cdd_generate_bindings_config_t *config) {
  FILE *f;
  char filepath[1024];
  const char *libname = config->library_name ? config->library_name : "libname";
...
int cdd_ffi_emit_csharp(cdd_ffi_ir_t *ir,
                        const cdd_generate_bindings_config_t *config) {
  int rc;
