 ...
 */
static int handle_doGetBranches(struct c_rest_request *req, struct c_rest_response *res, void *user_data) {
...
int main(int argc, char **argv) {
    ...
    { cdd_c_error_t rc = init_db(); if(rc != CDD_C_SUCCESS) return rc; }
...
static int handle_mcp_sse(struct c_rest_request *req, struct c_rest_response *res, void *user_data) {
    (void)req;
    (void)user_data;
    res->status_code = 200;
    /* c_rest_response_add_header is pseudo; just printing headers manually or using standard HTTP framework */
    return res->status_code;
    return CDD_C_SUCCESS;
}

static int handle_mcp_message(struct c_rest_request *req, struct c_rest_response *res, void *user_data) {
    (void)req;
    (void)user_data;
    res->status_code = 202;
    return res->status_code;
    return CDD_C_SUCCESS;
            ...
            c_rest_router_add(router, "GET", "/test/route", handle_doGetBranches, NULL);
