...
int write_lifecycle_funcs(FILE *h, FILE *c, const char *prefix,
  ...
  CHECK_IO(fprintf(c,
                   ...
                   " * @brief Auto-generated code from OpenAPI specification\n"
                   " */\n"
                   "int ApiError_from_json(const char *json, struct ApiError "
                   "**out) {\n"
                   "  JSON_Value *root;\n"
                   "  JSON_Object *obj;\n"
...
int openapi_client_generate(const struct OpenAPI_Spec *spec,
  ...
  if (config->model_header)
    ...
    model_h = (derive_model_header(base_name,
  ...
  if (config && !config->no_installable_package) {
    ...
    snprintf(hpath, sizeof(hpath), "%s/src/url_utils.h", dir_name ? dir_name : ".");
    snprintf(cpath, sizeof(cpath), "%s/src/url_utils.c", dir_name ? dir_name : ".");
...
" */\n"
"int url_query_add_encoded(struct UrlQueryParams *qp, const char *key,\n"
"                          const char *value) {\n"
...
"\n");
  ...
  if (config && config->create_tests_and_mocks) {
    ...
    if (tfp) {
      ...
      fprintf(tfp, "#if defined(__APPLE__)\n#include <c_abstract_http/http_apple.h>\n#define C_ABSTRACT_HTTP_INIT http_apple_context_init\n#define C_ABSTRACT_HTTP_SEND http_apple_send\n#else\n#include <c_abstract_http/http_curl.h>\n#define C_ABSTRACT_HTTP_INIT http_curl_context_init\n#define C_ABSTRACT_HTTP_SEND http_curl_send\n#endif\n");
      fprintf(tfp, "int main() {\n  struct HttpClient client;\n  C_ABSTRACT_HTTP_INIT(&client.transport);\n  client.send = C_ABSTRACT_HTTP_SEND;\n");
      ...
      for (size_t i = 0; i < spec->n_paths; ++i) {
        for (size_t j = 0; j < spec->paths[i].n_operations; ++j) {
           ...
           const char *method = "";
           if (op->verb == OA_VERB_GET) method = "GET";
           else if (op->verb == OA_VERB_POST) method = "POST";
           else if (op->verb == OA_VERB_PUT) method = "PUT";
           else if (op->verb == OA_VERB_DELETE) method = "DELETE";
           else method = "GET";
           
           fprintf(tfp, "  {\n    struct HttpRequest req;\n    http_request_init(&req);\n");
           fprintf(tfp, "    req.method = HTTP_%s;\n", method);
           if (op->verb == OA_VERB_POST || op->verb == OA_VERB_PUT || op->verb == OA_VERB_PATCH) {
               fprintf(tfp, "    req.body = (uint8_t*)\"{}\";\n    req.body_len = 2;\n");
               fprintf(tfp, "    struct HttpHeader *hdrs = malloc(sizeof(struct HttpHeader));\n    hdrs[0].key = \"Content-Type\";\n    hdrs[0].value = \"application/json\";\n    req.headers.headers = hdrs;\n    req.headers.count = 1;\n");
           }
           
           char formatted_path[512];
           snprintf(formatted_path, sizeof(formatted_path), "%s", spec->paths[i].route);
           char *p = formatted_path;
           while (*p) {
               if (*p == '{') {
                   *p = '1'; 
                   char *q = p + 1;
           ...
           fprintf(tfp, "    req.url = \"http://localhost:8080/v2%s\";\n", formatted_path);
           fprintf(tfp, "    struct HttpResponse *res = NULL;\n");
           fprintf(tfp, "    client.send(client.transport, &req, &res);\n");
           
           fprintf(tfp, "    if (res && res->status_code >= 200 && res->status_code < 300) {\n");
           
           if (op->n_responses > 0 && op->responses[0].schema.ref_name) {
               fprintf(tfp, "        struct %s *out = NULL;\n", op->responses[0].schema.ref_name);
               fprintf(tfp, "        if (res->body) {\n");
               fprintf(tfp, "            int rc = %s_from_json((const char*)res->body, &out);\n", op->responses[0].schema.ref_name);
               fprintf(tfp, "            if (rc != 0) { fprintf(stderr, \"Parse failed\\n\"); }\n");
               fprintf(tfp, "        }\n");
           }
           fprintf(tfp, "    } else { fprintf(stderr, \"Status %%d\\n\", res ? res->status_code : 0); exit(1); }\n");
