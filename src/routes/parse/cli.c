...
int migrate_cli_main(int argc, char **argv) {
  ...
  if (argc < 1) {
    fprintf(stderr, "Usage: cdd-c migrate <up|down|create> [args...]\n");
    return EXIT_FAILURE;
  }
  ...
  if (strcmp(subcmd, "up") == 0) {
    return run_pending_migrations(migrations_dir);
  } else if (strcmp(subcmd, "down") == 0) {
    return rollback_last_migration(migrations_dir);
  } else if (strcmp(subcmd, "create") == 0) {
    if (argc < 2) {
      fprintf(stderr, "Usage: cdd-c migrate create <name>\n");
      return EXIT_FAILURE;
    }
    return create_migration_file(migrations_dir, argv[1]);
  }

  fprintf(stderr, "Unknown migrate command: %s\n", subcmd);
  return EXIT_FAILURE;
}
 ...
 */
int db_reset_cli_main(int argc, char **argv) {
  const char *migrations_dir = "migrations";
  (void)argc;
  (void)argv;
  return reset_database(migrations_dir);
}

/**
 * @brief Executes the schema dump cli main operation.
 */
int schema_dump_cli_main(int argc, char **argv) {
  const char *out_file = "schema.sql";
  if (argc >= 1) {
    out_file = argv[0];
  }
  return dump_schema(out_file);
}
/**
 * @brief Executes the seed cli main operation.
 */
int seed_cli_main(int argc, char **argv) {
  const char *seed_file = "seeds.sql";
  if (argc >= 1) {
    seed_file = argv[0];
  }
  return seed_database(seed_file);
}

/**
 * @brief Executes the setup test db cli main operation.
 */
int setup_test_db_cli_main(int argc, char **argv) {
 ...
 */
int sql2c_main(int argc, char **argv) {
  const char *sql_file;
  const char *out_dir;
  char *sql_data;
  size_t sql_size;
  ...
  rc = read_to_file(sql_file, "rb", &sql_data, &sql_size);
  if (rc != 0)
    return EXIT_FAILURE;

  if (sql_size > 0 && sql_data) {
    sql_data[sql_size] = '\0';
  }
