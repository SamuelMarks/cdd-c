  ...
  }
  {
    ...
    for (i = 1; i < 50; i++) {
      ...
      tokenize(az_span_create_from_str((
                   char *)(size_t)"struct A { int a: 1; }; enum E { X }; union "
...
TEST parse_tokens_forward_declaration(void) {
  ...
  const az_span code =
      az_span_create_from_str((char *)(size_t)"struct MyStruct;");
...
TEST parse_tokens_anonymous_struct(void) {
  ...
  struct CstNodeList cst = {0};
  const az_span code =
      az_span_create_from_str((char *)(size_t)"struct { int x; };");
...
TEST parse_tokens_struct_variable_declaration(void) {
  ...
  struct CstNodeList cst = {0};
  const az_span code =
      az_span_create_from_str((char *)(size_t)"struct S { int x; } s;");
...
TEST parse_simple_array_init(void) {
  ...
  const az_span code =
      az_span_create_from_str((char *)(size_t)"int a[] = { 1, 2, 3 };");
...
TEST parse_compound_literal(void) {
  ...
  const az_span code =
      az_span_create_from_str((char *)(size_t)"var = (struct S){ .x = 1 };");
...
TEST parse_control_block_split(void) {
  ...
  const az_span code =
      az_span_create_from_str((char *)(size_t)"if (1) { x=1; }");
...
TEST parse_nested_compound_literal(void) {
  ...
  const az_span code =
      az_span_create_from_str((char *)(size_t)"func((struct Point){0,0});");
...
TEST parse_return_compound(void) {
  ...
  const az_span code =
      az_span_create_from_str((char *)(size_t)"return (int[]){1,2};");
...
TEST parse_c11_generic(void) {
  ...
  const az_span code = az_span_create_from_str(
      (char *)(size_t)"#define cbrt(X) _Generic((X), long double: cbrtl, "
  ...
  */
...

  {
    const az_span code2 = az_span_create_from_str(
        (char *)(size_t)"int x = _Generic(1.0, float: 1, default: 0);");
...
TEST test_cst_parser_extra(void) {
  ...
  /* NULL out_tree */
  {
    cdd_c_error_t out_rc =
        cdd_cst_parse(az_span_create_from_str((char *)""), NULL);
    ASSERT_EQ(CDD_C_ERROR_INVALID_ARGUMENT, out_rc);
  }

  {
    cdd_cst_tree_t *t_stray = NULL;
    cdd_cst_parse(az_span_create_from_str((char *)"}"), &t_stray);
  ...
  }
  {
    ...
    cdd_cst_parse(az_span_create_from_str((char *)"{}"), &t_empty);
    if (t_empty)
  ...
  }
  {
    cdd_cst_tree_t *t_empty = NULL;
    cdd_cst_parse(
        az_span_create_from_str((char *)(size_t)"void f() noexcept(true);"),
        &t_empty);
    if (t_empty)
      cdd_cst_tree_free(t_empty);
  }

  {
    ...
    cdd_cst_parse(az_span_create_from_str((char *)"#define D 1\n"), &t_macro);
    if (t_macro)
...
TEST parse_tokens_oom(void) {
#ifdef CDD_BUILD_TESTS
  {
    ...
    (void)rc;
    tokenize(az_span_create_from_str(
                 (char *)"int main() { char *p = malloc(10); return 0; }"),
             &tl);
...
TEST test_parse_tokens_attributes(void) {
  struct TokenList *tl = NULL, *tl2 = NULL, *tl3 = NULL, *tl4 = NULL,
                   *tl5 = NULL;
  struct CstNodeList cst = {0};
  tokenize(az_span_create_from_str((char *)"[[nodiscard]] int x;"), &tl);
  ASSERT_EQ(0, parse_tokens(tl, &cst));
  free_cst_node_list(&cst);
  free_token_list(tl);

  tokenize(az_span_create_from_str((char *)"__attribute__((unused)) int x;"),
           &tl5);
  ...
  tokenize(
      az_span_create_from_str((char *)"[[unknown_attr(1, 2, 3)]] void f() {}"),
      &tl2);
  ASSERT_EQ(0, parse_tokens(tl2, &cst));
  free_cst_node_list(&cst);
  free_token_list(tl2);

  tokenize(az_span_create_from_str((char *)"[["), &tl3);
  ASSERT_EQ(0, parse_tokens(tl3, &cst));
  free_cst_node_list(&cst);
  free_token_list(tl3);

  tokenize(az_span_create_from_str((char *)"[[unknown_attr[1]]] void f() {}"),
           &tl4);
  {
    tl = NULL;
    memset(&cst, 0, sizeof(cst));
    tokenize(az_span_create_from_str((char *)"__attribute__((always_inline))"),
             &tl);
    ASSERT_EQ(0, parse_tokens(tl, &cst));
    free_cst_node_list(&cst);
    free_token_list(tl);
  }
  {
    tl = NULL;
    memset(&cst, 0, sizeof(cst));
    tokenize(az_span_create_from_str((char *)"__declspec(dllexport)"), &tl);
    ASSERT_EQ(0, parse_tokens(tl, &cst));
    free_cst_node_list(&cst);
    free_token_list(tl);

    tokenize(az_span_create_from_str((char *)"__declspec(align(16)) int x;"),
             &tl);
    ASSERT_EQ(0, parse_tokens(tl, &cst));
    free_cst_node_list(&cst);
    free_token_list(tl);
...
TEST test_parse_tokens_static_assert(void) {
  struct TokenList *tl = NULL, *tl2 = NULL, *tl3 = NULL, *tl4 = NULL,
                   *tl5 = NULL;
  struct CstNodeList cst = {0};
  tokenize(az_span_create_from_str((char *)"_Static_assert(1 == 1, \"msg\");"),
           &tl);
  ASSERT_EQ(0, parse_tokens(tl, &cst));
  free_cst_node_list(&cst);
  free_token_list(tl);

  tokenize(az_span_create_from_str((char *)"_Static_assert(1 == 1);"), &tl2);
  ASSERT_EQ(0, parse_tokens(tl2, &cst));
  free_cst_node_list(&cst);
  free_token_list(tl2);

  tokenize(az_span_create_from_str((char *)"_Static_assert"), &tl3);
  ASSERT_EQ(0, parse_tokens(tl3, &cst));
  free_cst_node_list(&cst);
  free_token_list(tl3);

  tokenize(
      az_span_create_from_str((char *)"_Static_assert((1 == 1), \"msg\");"),
      &tl4);
  ASSERT_EQ(0, parse_tokens(tl4, &cst));
  free_cst_node_list(&cst);
  free_token_list(tl4);

  tokenize(az_span_create_from_str((char *)"_Static_assert(1 == 1)"), &tl5);
  ASSERT_EQ(0, parse_tokens(tl5, &cst));
  free_cst_node_list(&cst);
  free_token_list(tl5);

  {
    ...
    tokenize(az_span_create_from_str((char *)"_Static_assert(1 == 1;"), &tl6);
