...
TEST test_cdd_cst_eval_sizeof_alignof_advanced(void) {
  ...
  rc = cdd_cst_parse(
      az_span_create_from_str((char *)(size_t) "struct Unknown x;"), &tree2);
  ASSERT_EQ(0, rc);
  ...
  /* What if extract_type_name has multiple spaces? */
  {
    ...
    rc = cdd_cst_parse(
        az_span_create_from_str((char *)(size_t) "unsigned long long int a;"),
        &tree3);
    ASSERT_EQ(0, rc);
  ...
  /* What if extract_type_name buffer overflows? (256 bytes) */
  {
    ...
    huge_type[292] = ';';
    huge_type[293] = '\0';
    rc = cdd_cst_parse(az_span_create_from_str((char *)(size_t)huge_type),
                       &tree4);
    ASSERT_EQ(0, rc);
...
SUITE(cdd_cst_type_eval_suite) {
  RUN_TEST(test_cdd_cst_eval_ptr_fail);
  ...
  RUN_TEST(test_cdd_cst_type_eval_branches);
}
