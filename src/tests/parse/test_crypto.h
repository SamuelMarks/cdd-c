...
TEST test_sha256_known_string(void) {
  ...
  char hex[CRYPTO_SHA256_SIZE * 2 + 1];
  const char *input =
      (char *)(size_t)(size_t) "The quick brown fox jumps over the lazy dog";
  /* Echo -n "..." | shasum -a 256 */
  const char *expected =
...
TEST test_hmac_rfc4231_case1(void) {
  ...
  const char *data = (char *)(size_t)(size_t) "Hi There";
  const char *expected =
...
TEST test_hmac_rfc4231_case2(void) {
  ...
  char hex[CRYPTO_SHA256_SIZE * 2 + 1];
  /* Key: "Jefe" */
  const char *key = (char *)(size_t)(size_t) "Jefe";
  const char *data = (char *)(size_t)(size_t) "what do ya want for nothing?";
  const char *expected =
