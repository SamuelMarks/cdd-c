...
TEST test_weaver_wrap_ifdef_else(void) {
  ...
  res = weaver_wrap_ifdef(&patches, tokens, 0, tokens->size, "_MSC_VER", "int a = 10;");
  ASSERT_EQ(0, res);
...
TEST test_weaver_vla_to_alloca(void) {
  ...
  for (i = 0; i < tokens->size; ++i) {
    if (tokens->tokens[i].kind == TOKEN_KEYWORD_INT) {
      ...
      if (j < tokens->size && tokens->tokens[j].kind == TOKEN_IDENTIFIER) {
        ...
        if (is_arr) {
          start_idx = i;
          while (j < tokens->size && tokens->tokens[j].kind != TOKEN_SEMICOLON) j++;
          end_idx = j + 1;
          break;
        }
      }
  ...
  res = weaver_vla_to_alloca(&patches, tokens, start_idx, end_idx, "int", "arr", "n", false);
  ASSERT_EQ(0, res);

  res = patch_list_apply(&patches, tokens, &out_code);
  ASSERT_EQ(0, res);

  ASSERT_STR_EQ("int main() {
  int n = 10;
