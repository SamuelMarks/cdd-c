...
static void run_io_loop(const struct OpenAPI_Operation *op, const char *tmpl,
  ...
  while (1) {
    ...
    if (op) {
      ...
      gen_url_code(tmpl, params, n_params, &code);
    }
    if (code) {
      free(code);
      break;
