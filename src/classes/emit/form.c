...
write_struct_to_form_urlencoded_func(FILE *fp, const char *struct_name,
  ...
  fprintf(fp,
          "static cdd_c_error_t %s_url_encode_form(const char *str, char "
          "**out) {\n",
          struct_name);
  fprintf(fp, "  size_t len = 0, idx = 0, out_idx = 0;\n");
  fprintf(fp, "  char *res;\n");
  fprintf(fp, "  if (!str || !out) return CDD_C_ERROR_MEMORY;\n");
  ...
  fprintf(fp, "    }\n");
  fprintf(fp, "  }\n");
  fprintf(fp, "  res = (char *)(size_t)malloc(len + 1);\n");
  ...
  fprintf(fp, "/**\n * @brief Autogenerated Struct-to-Form automatic "
              "generation.\n */\n");
  fprintf(fp,
          "cdd_c_error_t %s_to_form_urlencoded(const struct %s *obj, char "
          "**out) {\n",
  ...
  fprintf(fp, "  res = (char *)(size_t)malloc(capacity);\n");
  fprintf(fp, "  if (!res) return CDD_C_ERROR_MEMORY;\n");
  fprintf(fp, "  res[0] = '\\0';\n\n");
  ...
  for (i = 0; i < sf->size; ++i) {
    ...
    if (strcmp(f->type, "string") == 0) {
      ...
      fprintf(fp, "        tmp = (char *)(size_t)realloc(res, capacity);\n");
