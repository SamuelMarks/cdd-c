 ...
 */
int cdd_strndup2(const char *s, size_t n, char * *_out_val) {
  char *result;
  size_t len = 0;
  while (len < n && s[len] != '\0')
    len++;
 ...
 * @brief Autogenerated docstring
 */
int add_option(struct CliCommand *cmd, struct CliOption * *_out_val) {
...
int cst_extract_cli_command(const struct CstNodeList *nodes,
                            ...
                            struct CliCommand *cmd) { struct CliOption * _ast_add_option_0; char * _ast_cdd_strndup2_1; 
  size_t i, j;
  int in_getopt = 0;
      ...
      if (len == 6 && strncmp(str, "getopt", 6) == 0) {
        ...
        for (j = i + 1; j < i + 10 && j < tokens->size; ++j) {
          if (tokens->tokens[j].kind == TOKEN_STRING_LITERAL) {
            ...
            for (k = 0; k < flen; ++k) {
              if (isalpha(flags[k])) {
                struct CliOption *opt = (add_option(cmd, &_ast_add_option_0), _ast_add_option_0);
              ...
              for (j = i + 2; j < tokens->size && tokens->tokens[j].kind != TOKEN_KEYWORD_BREAK; ++j) {
                if (tokens->tokens[j].kind == TOKEN_ASSIGN) {
                  if (j > 0 && tokens->tokens[j-1].kind == TOKEN_IDENTIFIER) {
                    cmd->options[opt_idx].mapped_struct_field = (cdd_strndup2((const char*)tokens->tokens[j-1].start, tokens->tokens[j-1].length, &_ast_cdd_strndup2_1), _ast_cdd_strndup2_1);
                  }
                  break;
                }
              }
