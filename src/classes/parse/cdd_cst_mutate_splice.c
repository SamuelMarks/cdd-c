...
int cdd_cst_splice_children(cdd_cst_tree_t *tree, cdd_cst_node_t **node_ptr, size_t start_idx, size_t consume_count, cdd_cst_child_t *new_children, size_t new_children_count) {
    ...
    if (rc == 0 && node_ptr) *node_ptr = new_node;
    return rc;
}

cdd_cst_node_t *cdd_cst_find_node_for_token(cdd_cst_node_t *root, cdd_token_t *tok, size_t *out_idx) {
    size_t i;
    if (!root || !tok || !out_idx) return NULL;

    for (i = 0; i < root->num_children; i++) {
        if (root->children[i].kind == CDD_CST_CHILD_TOKEN && root->children[i].val.token == tok) {
            *out_idx = i;
            return root;
        } else if (root->children[i].kind == CDD_CST_CHILD_NODE) {
            cdd_cst_node_t *found = cdd_cst_find_node_for_token(root->children[i].val.node, tok, out_idx);
